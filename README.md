...
- Strongly suggest install Anaconda and launch JupyterNotebook to run the code.


## Some notes:
* Since the whole process time took nearly 3 hours to scrape all the raw dataset from the web, I have set the code to saved the raw dataset into .csv file and reloaded them when required.

* !!!! If you want to run the data scraping part of the code, please, change output file directory to avoid overwriting the existing data in my original .csv file.

## How to set parameters:
1. Use your own browser to enter the page in Figure 1, and then replace the values of the **place_id** and **federated_search_session_id** fields in **url** under the **main.py** file **base_url** the corresponding field value in the variable, that's it.

2. The **user-agent** field in the headers variable in the next line of **base_url** can also be replaced with the value of **user-agent** of your own browser.

3. Here is a template of my crawler, but the currency is expressed in CNY. When starting the analysis, complete step 1 first, and then crawl it again by yourself. What you get is the local currency.

4. The name of the data captured in Figure 2 (title, mark, etc.) can also be modified at line 47 of the code in **main.py**.
